...
private _requiredWeightRatio = _objective get "requiredWeightRatio";
...
if (!_eastQualifies && !_westQualifies) then {
    if (!(_oldOwner isEqualTo sideUnknown) && {!(_anchorOwner isEqualTo _oldOwner)}) then {
...
if (!(_oldOwner isEqualTo _newOwner)) then {
    _objective set ["lastChanged", diag_tickTime];
    diag_log format [
        "[FLO][Objective] Objective %1 owner changed from %2 to %3",
        _objective get "id",
...
!(_oldOwner isEqualTo _newOwner) || {!(_oldState isEqualTo _newState)}
