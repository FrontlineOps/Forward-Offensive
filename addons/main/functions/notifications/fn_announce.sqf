...
private _h = 0.132 * safeZoneH;
private _x = safeZoneX + ((safeZoneW - _w) / 2);
private _y = safeZoneY + (0.115 * safeZoneH);
private _padX = 0.018 * safeZoneW;
...
FLO_AnnouncementControls = [_group, _bg, _topLine, _sideLine, _header, _body];
...
_sideLine ctrlSetPosition [0, 0, 0.006 * safeZoneW, _h];
_sideLine ctrlSetBackgroundColor _accentColor;
_sideLine ctrlCommit 0;

_header ctrlSetPosition [_padX, 0.018 * safeZoneH, _w - (_padX * 2), 0.042 * safeZoneH];
_header ctrlSetStructuredText (parseText format [
    "<t color='%1' size='1.20'>%2</t>",
    _accentHtml,
    toUpper _title
]);
_header ctrlCommit 0;

_body ctrlSetPosition [_padX, 0.064 * safeZoneH, _w - (_padX * 2), 0.055 * safeZoneH];
_body ctrlSetStructuredText (parseText format [
    "<t color='#F2F7FA' size='0.95'>%1</t><br/><t color='%2' size='0.66'>%3</t>",
    _message,
    _accentHtml,
    _style get "label"
]);
